...
def _Read(p: pathlib.Path) -> str:
  try:
    ...
    raise SystemExit(f'error: file not found: {p}') from err
...
def _InjectIncludes(readme_path: str = 'README.md', repo_root: str = '.') -> int:
  root: pathlib.Path = pathlib.Path(repo_root).resolve()
  readme: pathlib.Path = root / readme_path
  orig: str = _Read(readme)
  changed: bool = False
  ...
  def _Repl(match: re.Match[str]) -> str:
    ...
    src = root / rel_path
    content: str = _Read(src)
    # ensure a clean newline sandwich between markers
    injected: str = f'{match.group("prefix")}\n{content.rstrip()}\n{match.group("suffix")}'
    changed = True
    return injected

  new: str = INCLUDE_BLOCK_RE.sub(_Repl, orig)
  if new == orig:
