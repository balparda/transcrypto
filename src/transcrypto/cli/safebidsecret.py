...
@sss_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p sss-key sss rawshares 999 5\n\n'
...
def SSSRawShares(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  secret_i: int = safecrypto.ParseInt(secret, min_value=1)
  for i, share in enumerate(sss_priv.RawShares(secret_i, max_shares=count)):
    safecrypto.SaveObj(share, f'{base_path}.share.{i + 1}', config.protect)
  config.console.print(
...
def SSSRawRecover(*, ctx: click.Context) -> None:  # documentation is help/epilog/args # noqa: D103
  config: safecrypto.TransConfig = ctx.obj
  base_path: str = safecrypto.RequireKeyPath(config, 'sss')
  sss_pub: sss.ShamirSharedSecretPublic = safecrypto.LoadObj(
  ...
  for fname in glob.glob(base_path + '.share.*'):  # noqa: PTH207
    subset.append(safecrypto.LoadObj(fname, config.protect, sss.ShamirSharePrivate))
...
@sss_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p sss-key sss rawverify 999\n\n'
    ...
    '$ poetry run safecrypto -p sss-key sss rawverify 998\n\n'
