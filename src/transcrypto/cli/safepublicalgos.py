...
@rsa_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p rsa-key.pub rsa rawencrypt 999\n\n'
    '6354905961171348600'
  ),
)
@clibase.CLIErrorGuard
def RSARawEncrypt(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  config: safecrypto.TransConfig = ctx.obj
  message_i: int = safecrypto.ParseInt(message, min_value=1)
  key_path: str = safecrypto.RequireKeyPath(config, 'rsa')
  rsa_pub: rsa.RSAPublicKey = rsa.RSAPublicKey.Copy(
    safecrypto.LoadObj(key_path, config.protect, rsa.RSAPublicKey)
  )
...
@rsa_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p rsa-key.priv rsa rawdecrypt 6354905961171348600\n\n'
    '999'
  ),
)
@clibase.CLIErrorGuard
def RSARawDecrypt(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  config: safecrypto.TransConfig = ctx.obj
  ciphertext_i: int = safecrypto.ParseInt(ciphertext, min_value=1)
  key_path: str = safecrypto.RequireKeyPath(config, 'rsa')
...
@rsa_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n$ poetry run safecrypto -p rsa-key.priv rsa rawsign 999\n\n7632909108672871784'
  ),
)
@clibase.CLIErrorGuard
...
@rsa_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p rsa-key.pub rsa rawverify 999 7632909108672871784\n\n'
    'RSA signature: OK\n\n'
    '$ poetry run safecrypto -p rsa-key.pub rsa rawverify 999 7632909108672871785\n\n'
...
def RSARawVerify(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  config: safecrypto.TransConfig = ctx.obj
  message_i: int = safecrypto.ParseInt(message, min_value=1)
  signature_i: int = safecrypto.ParseInt(signature, min_value=1)
  key_path: str = safecrypto.RequireKeyPath(config, 'rsa')
  rsa_pub: rsa.RSAPublicKey = rsa.RSAPublicKey.Copy(
    safecrypto.LoadObj(key_path, config.protect, rsa.RSAPublicKey)
  )
...
safecrypto.app.add_typer(eg_app, name='elgamal')
...
@eg_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p eg-key elgamal shared --bits 64  '
    '# NEVER use such a small key: example only!\n\n'
...
def ElGamalShared(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  config: safecrypto.TransConfig = ctx.obj
  base_path: str = safecrypto.RequireKeyPath(config, 'elgamal')
  shared_eg: elgamal.ElGamalSharedPublicKey = elgamal.ElGamalSharedPublicKey.NewShared(bits)
  safecrypto.SaveObj(shared_eg, base_path + '.shared', config.protect)
...
def ElGamalNew(*, ctx: click.Context) -> None:  # documentation is help/epilog/args # noqa: D103
  config: safecrypto.TransConfig = ctx.obj
  base_path: str = safecrypto.RequireKeyPath(config, 'elgamal')
  shared_eg: elgamal.ElGamalSharedPublicKey = safecrypto.LoadObj(
  ...
  safecrypto.SaveObj(eg_priv, base_path + '.priv', config.protect)
  safecrypto.SaveObj(eg_pub, base_path + '.pub', config.protect)
...
@eg_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p eg-key.pub elgamal rawencrypt 999\n\n'
...
def ElGamalRawEncrypt(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  config: safecrypto.TransConfig = ctx.obj
  message_i: int = safecrypto.ParseInt(message, min_value=1)
  key_path: str = safecrypto.RequireKeyPath(config, 'elgamal')
  eg_pub: elgamal.ElGamalPublicKey = elgamal.ElGamalPublicKey.Copy(
    safecrypto.LoadObj(key_path, config.protect, elgamal.ElGamalPublicKey)
  )
  c1: int
  c2: int
...
@eg_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p eg-key.priv elgamal rawdecrypt '
    '2948854810728206041:15945988196340032688\n\n'
    '999'
  ),
)
...
def ElGamalRawDecrypt(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  config: safecrypto.TransConfig = ctx.obj
  ciphertext_i: tuple[int, int] = safecrypto.ParseIntPairCLI(ciphertext)
  key_path: str = safecrypto.RequireKeyPath(config, 'elgamal')
  eg_priv: elgamal.ElGamalPrivateKey = safecrypto.LoadObj(
...
@eg_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p eg-key.priv elgamal rawsign 999\n\n'
...
@eg_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p eg-key.pub elgamal rawverify 999 '
    '4674885853217269088:14532144906178302633\n\n'
...
def ElGamalRawVerify(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  config: safecrypto.TransConfig = ctx.obj
  message_i: int = safecrypto.ParseInt(message, min_value=1)
  signature_i: tuple[int, int] = safecrypto.ParseIntPairCLI(signature)
  key_path: str = safecrypto.RequireKeyPath(config, 'elgamal')
  eg_pub: elgamal.ElGamalPublicKey = elgamal.ElGamalPublicKey.Copy(
    safecrypto.LoadObj(key_path, config.protect, elgamal.ElGamalPublicKey)
  )
...
@eg_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -i bin -o b64 -p eg-key.pub elgamal encrypt "abcde" -a "xyz"\n\n'
...
def ElGamalEncrypt(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  config: safecrypto.TransConfig = ctx.obj
  key_path: str = safecrypto.RequireKeyPath(config, 'elgamal')
  eg_pub: elgamal.ElGamalPublicKey = safecrypto.LoadObj(
...
@eg_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -i b64 -o bin -p eg-key.priv elgamal decrypt -a eHl6 -- '
...
@eg_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -i bin -o b64 -p eg-key.priv elgamal sign "xyz"\n\n'
...
@eg_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -i b64 -p eg-key.pub elgamal verify -- eHl6 '
    'Xl4hlYK8SHVGw…0fCKJE1XVzA==\n\n'  # cspell:disable-line
    'El-Gamal signature: OK\n\n'
    '$ poetry run safecrypto -i b64 -p eg-key.pub elgamal verify -- eLl6 '
    'Xl4hlYK8SHVGw…0fCKJE1XVzA==\n\n'  # cspell:disable-line
    'El-Gamal signature: INVALID'
  ),
)
...
@dsa_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p dsa-key.priv dsa rawsign 999\n\n'
...
@dsa_app.command(
  ...
  epilog=(
    'Example:\n\n\n\n'
    '$ poetry run safecrypto -p dsa-key.pub dsa rawverify 999 2395961484:3435572290\n\n'
    'DSA signature: OK\n\n'
    '$ poetry run safecrypto -p dsa-key.pub dsa rawverify 999 2395961484:3435572291\n\n'
...
def DSARawVerify(  # documentation is help/epilog/args # noqa: D103
  *,
  ...
  dsa_pub: dsa.DSAPublicKey = dsa.DSAPublicKey.Copy(
    safecrypto.LoadObj(key_path, config.protect, dsa.DSAPublicKey)
  )
  message_i: int = safecrypto.ParseInt(message, min_value=1)
  signature_i: tuple[int, int] = safecrypto.ParseIntPairCLI(signature)
