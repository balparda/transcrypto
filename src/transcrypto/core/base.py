...
def BetaIncomplete(a: float, b: float, x: float, /) -> float:
  ...
  Raises:
    InputError: If x is outside [0, 1].
  ...
  """
  if x < 0.0 or x > 1.0:
    raise InputError(f'x must be in [0, 1], got {x}')
  if x == 0.0:
    return 0.0
  if x == 1.0:
    return 1.0
...
def StudentTPPF(q: float, df: float, /) -> float:
  ...
  """
  if not 0.0 < q < 1.0:
    raise InputError(f'q must be in (0, 1), got {q}')
...
def SampleVariance(data: list[int | float], mean: float, /) -> float:
  ...
  Raises:
    InputError: If len(data) < 2.

  """
  if (data_sz := len(data)) < 2:  # noqa: PLR2004
    raise InputError(f'sample variance requires at least 2 data points, got {data_sz}')
