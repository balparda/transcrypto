...
class SymmetricCrypto(abc.ABC):
  ...
  Contract:
    - Implementations must be deterministic *only* if a nonce is provided.
    - If `nonce` is omitted, the implementation should generate a fresh, safe nonce.
    - If AEAD is supported, `associated_data` (AAD) must be authenticated.
  ...
  def Encrypt(
      ...
      nonce: bytes | None = None, associated_data: bytes | None = None) -> bytes:
    """Encrypt `plaintext` and return `ciphertext`.

    Args:
      plaintext (bytes): Data to encrypt.
      nonce (bytes, optional): Optional nonce/IV; if None, and nonces are part of
          the protocol, generate a safe random nonce internally and embed/return it
          as part of the ciphertext format
  ...
  def Decrypt(
    ...
    Args:
      ...
      nonce (bytes, optional): Optional nonce/IV if the format does not embed it.
