...
r"""Balparda's SafeTrans(Crypto) command line interface: the safe version of TransCrypto.
...
Great question — crypto CLIs juggle a messy mix of bytes, strings, and files. The trick is to make
...
Below is a battle-tested pattern (inspired by OpenSSL, age, gpg, minisign, libsodium tools) plus a
compact argparse skeleton you can drop in.
...
When outputs are multi-field (e.g., keygen with pub+priv), offer --out-json with stable field names
and base64/hex encodings. Pair with --out-prefix for file emission.
...
How other tools approach it (patterns to copy)
  •  OpenSSL: -in/-out for files; PEM/DER auto-detected by headers; -passin schemes (file:, env:) —
  good precedent for scheme-style sources.
  •  GnuPG: --armor (base64) vs raw; refuses to dump secrets to TTY without explicit flags;
  separate --output vs stdout.
  •  age / minisign: simple verbs, -i for identity (key file), -a/--armor for base64, defaults to
  stdout for single outputs.
  •  libsodium CLI: clear distinction between --bin, --hex, --base64 across commands.

These all lean on: consistent flags, scheme prefixes for inputs, explicit armor flags,
and safe defaults.
...
Gotchas & tips
  ...
  •  Stateful formats: PEM/JWK support is valuable; expose --key-format {raw,pem,jwk} and
...
If you want, tell me a couple of concrete operations you'll ship first
(e.g., ed25519 sign/verify, chacha20-poly1305 enc/dec, hkdf), and I'll tailor the subparsers and
validators for those right away.

Awesome list. Here's a clean, consistent CLI contract for each item—built on the “DataSpec +
output/sink” pattern we discussed. I'll give you: (1) command shapes with flags, inputs/outputs,
and exit-code behavior, and (2) an argparse scaffold you can paste in and extend.
...
1) isprime
  ...
  •  Exit codes: 0 always if test ran; optionally --expect {prime,composite} makes exit 0 only if
  matched, else 2.
...
Generates randomness in various shapes.
  ...
  •  Output: single blob (respect --out-*). For int, if textual, print base-10; if --out-hex,
  print hex; --out-raw prints big-endian bytes.
...
Symmetric crypto. Keep keys as DataSpec.
  ...
  •  --key DataSpec or --from-pass str:PASSWORD --kdf {argon2id,pbkdf2} [--salt DataSpec]
  [--iters N] [--mem-mb N] [--parallelism N]
...
Small note: “DSA” is for signing; shared-secret is “DH/EC Diffie-Hellman.” I'll split them:
  ...
  •  Output: shared secret bytes (blob). Often you'll immediately feed this to HKDF; consider
  --kdf … --bytes N that outputs a KDF'd key instead of raw shared secret.

7) bid

Ambiguous name. Two likely meanings: blind signatures (e.g., RSA-blind for anonymous credentials) or
binary ID/address generator. I'll make it a namespace you can fill later; the interface below works
for blind signatures:
  •  bid new --alg rsa --bits N → issuer keys (like rsa new)
  •  bid sign --key DataSpec --msg DataSpec --blind → blind signature flow (you may need a
  --token DataSpec and a --context str:…)
  •  bid verify --pub DataSpec --msg DataSpec --sig DataSpec
If that's not what you meant, keep the namespace and plug the real semantics in; the IO pattern
(DataSpec in, blob/json out) still fits.

8) sss (Shamir's Secret Sharing)
  •  Subcommands:
  •  sss new --secret DataSpec --threshold K --shares N [--id str:label]
  •  Output:
  •  --out-prefix PREFIX → writes PREFIX.1, PREFIX.2, … each a compact share file (recommend binary
  with header; also allow --armor for base64)
...
def build_parser():
  ...
  dsa_verify.add_argument('--pub', required=True); dsa_verify.add_argument('--msg', required=True);
  dsa_verify.add_argument('--sig', required=True)
  ...
  dh_shared.add_argument('--group', help='ffdhe… or secp…');
  dh_shared.add_argument('--curve', help='x25519, secp256r1, …')
  ...
  # bid (placeholder)
  p_bid = sub.add_parser('bid', help='blind-id namespace');
  sbid = p_bid.add_subparsers(dest='sub', required=True)
  bid_new = sbid.add_parser('new', help='issuer keys');
  add_shared_output(bid_new); bid_new.set_defaults(func=cmd_bid_new)
  bid_verify = sbid.add_parser('verify', help='verify token'); add_shared_output(bid_verify)
  bid_verify.add_argument('--pub', required=True); bid_verify.add_argument('--msg', required=True);
  bid_verify.add_argument('--sig', required=True)
  bid_verify.set_defaults(func=cmd_bid_verify)

  # sss
  p_sss = sub.add_parser('sss', help='Shamir secret sharing');
  ssss = p_sss.add_subparsers(dest='sub', required=True)
  sss_new = ssss.add_parser('new', help='split secret'); add_shared_output(sss_new)
  sss_new.add_argument('--secret', required=True);
  sss_new.add_argument('--threshold', type=int, required=True);
  sss_new.add_argument('--shares', type=int, required=True); sss_new.add_argument('--id')
...
If you want, I can fill in concrete implementations next (e.g., Miller-Rabin for isprime, X25519
for dh shared, AES-GCM JSON bundle, RSA PSS, SSS split/recover). Tell me which two or three
you want first and I'll wire them up.
...
def _BuildParser() -> argparse.ArgumentParser:  # noqa: PLR0914, PLR0915
  ...
  parser: argparse.ArgumentParser = argparse.ArgumentParser(
    ...
    description=(
      'safetrans: CLI for safe random, primes, hashing, AES, RSA, DSA, bidding, and secret sharing.'
    ),
    epilog=(
      'Examples:\n\n'
      '  # --- Randomness / Primes ---\n'
      ...
      '  poetry run transcrypto --b64 --out-bin -p rsa-key.priv rsa decrypt -a <aad> -- '
      '<ciphertext>\n'
  ...
  parser.add_argument(
    ...
    default='',
    help=(
      'Input: "int" is (decimal) integer, "hex" is hexadecimal, '
  ...
  parser.add_argument(
    ...
    default='',
    help=(
      'Output: "int" is (decimal) integer, "hex" is hexadecimal, '
      '"b64" is base-64 encoded, "bin" is binary, '
      'anything else is considered a "/file/path" or "/file/path/prefix" to be used to '
  ...
  p_rand_bits: argparse.ArgumentParser = rsub.add_parser(
    'bits',
    help=(
      'Random bytes with exact bit length `bits` (≥ 8, MSB will be 1). '
  ...
  p_rand_bytes: argparse.ArgumentParser = rsub.add_parser(
    'bytes',
    help=(
      'Random bytes with exact bit length `n` (≥ 1, MSB will be 1). '
      'By default, `-o/--out` is hexadecimal.'
    ),
  ...
  p_rand_prime: argparse.ArgumentParser = rsub.add_parser(
    'prime',
    help=(
      'Random prime with exact bit length `bits` (≥ 11, MSB will be 1). '
      'By default, `-o/--out` is hexadecimal.'
    ),
  ...
  p_isprime.add_argument('n', type=str, help='Integer to test, ≥ 1')
  ...
  p_hf: argparse.ArgumentParser = hash_sub.add_parser(
    ...
    epilog=(
      'hash file /etc/passwd --digest sha512\n'
  ...
  p_aes_enc: argparse.ArgumentParser = aes_sub.add_parser(
    ...
    epilog=(
      '--b64 --out-b64 aes encrypt -k DbWJ_ZrknLEEIoq_NpoCQwHYfjskGokpueN2O_eY0es= -- '  # cspell:disable-line
  ...
  p_rsa_enc_safe: argparse.ArgumentParser = rsa_sub.add_parser(
    ...
    epilog=(
      '--bin --out-b64 -p rsa-key.pub rsa encrypt "abcde" -a "xyz"\nAO6knI6xwq6TGR…Qy22jiFhXi1eQ=='
    ),
  )
  ...
  p_rsa_dec_safe: argparse.ArgumentParser = rsa_sub.add_parser(
    ...
    epilog=(
      '--b64 --out-bin -p rsa-key.priv rsa decrypt -a eHl6 -- AO6knI6xwq6TGR…Qy22jiFhXi1eQ==\nabcde'
    ),
  )
  ...
  p_rsa_ver_safe: argparse.ArgumentParser = rsa_sub.add_parser(
    ...
    epilog=(
      '--b64 -p rsa-key.pub rsa verify -- eHl6 '
  ...
  p_dsa_verify_safe: argparse.ArgumentParser = dsa_sub.add_parser(
    ...
    help='Verify `signature` for `message` with public key.',
    epilog=(
      '--b64 -p dsa-key.pub dsa verify -- eHl6 yq8InJVpViXh9…BD4par2XuA=\n'
      'DSA signature: OK $$ '
