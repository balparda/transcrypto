...
def test_RandBits() -> None:
  ...
  with pytest.raises(base.InputError, match='n_bits must be ≥ 8'):
    saferandom.RandBits(7)
  gn: set[int] = set()
  gn.update(saferandom.RandBits(10) for _ in range(50))
  assert len(gn) > 30  # has a chance of 1 in 531,000 to fail
  gn = set()
  gn.update(saferandom.RandBits(10000) for _ in range(20))
...
def test_RandBits_bit_length_and_bias() -> None:
  ...
  for n_bits in (8, 17, 64, 4096):
    xs: list[int] = [saferandom.RandBits(n_bits) for _ in range(4000)]
...
def test_RandInt() -> None:
  ...
  with pytest.raises(base.InputError, match='min_int must be ≥ 0, and < max_int'):
    saferandom.RandInt(-1, 1)
  with pytest.raises(base.InputError, match='min_int must be ≥ 0, and < max_int'):
    saferandom.RandInt(2, 1)
  with pytest.raises(base.InputError, match='min_int must be ≥ 0, and < max_int'):
    saferandom.RandInt(2, 2)
  gn: set[int] = set()
  gn.update(saferandom.RandInt(10, 20) for _ in range(200))
  assert min(gn) == 10
  assert max(gn) == 20
...
def test_RandInt_uniform_small_range() -> None:
  ...
  counts: collections.Counter[int] = collections.Counter(
    saferandom.RandInt(10, 20) for _ in range(n)
  )
...
def test_RandBytes() -> None:
  ...
  with pytest.raises(base.InputError, match='n_bytes must be ≥ 1'):
    saferandom.RandBytes(0)
  assert len(saferandom.RandBytes(1)) == 1
  assert len(saferandom.RandBytes(1000)) == 1000
  assert (
    len({saferandom.RandBytes(32) for _ in range(100)}) == 100
  )  # chance of failure is 1 in 10**74
...
def test_RandBits_RandInt_RandShuffle_parallel_smoke() -> None:
  ...
  with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
    xs: list[int] = list(ex.map(lambda _: saferandom.RandBits(256), range(200)))  # pyright: ignore
    ys: list[int] = list(ex.map(lambda _: saferandom.RandInt(0, 1000), range(200)))  # pyright: ignore
    zs: list[bytes] = list(ex.map(lambda _: saferandom.RandBytes(32), range(200)))  # pyright: ignore
    seq: list[int] = list(range(50))
    # shuffle some independent copies
    list(ex.map(lambda _: saferandom.RandShuffle(seq[:]), range(50)))  # pyright: ignore
