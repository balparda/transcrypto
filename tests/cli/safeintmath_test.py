...
def test_rand_bits_properties() -> None:
  ...
  res: typer.testing.Result = safetrans_test._CallCLI(['random', 'bits', '16'])
...
def test_rand_int_properties() -> None:
  ...
  res: typer.testing.Result = safetrans_test._CallCLI(['random', 'int', '5', '9'])
  assert res.exit_code == 0
  n = int(safetrans_test.OneToken(res))
  assert 5 <= n <= 9
...
def test_rand_bytes_shape() -> None:
  ...
  res: typer.testing.Result = safetrans_test._CallCLI(['random', 'bytes', '4'])
  assert res.exit_code == 0
...
def test_cli_random_int_invalid_range_prints_error() -> None:
  ...
  res: typer.testing.Result = safetrans_test._CallCLI(['random', 'int', '9', '5'])
  assert res.exit_code == 0
...
def test_random_prime_properties(bits: int) -> None:
  ...
  res: typer.testing.Result = safetrans_test._CallCLI(['random', 'prime', str(bits)])
  assert res.exit_code == 0
