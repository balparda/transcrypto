...
def test_rsa_encrypt_decrypt_and_sign_verify(tmp_path: pathlib.Path) -> None:
  ...
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'rsa', 'rawencrypt', str(msg)])
  assert res.exit_code == 0
  c = int(safecrypto_test.OneToken(res))
  assert c > 0
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'rsa', 'rawdecrypt', str(c)])
  assert res.exit_code == 0
  assert int(safecrypto_test.OneToken(res)) == msg
  # Sign/verify
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'rsa', 'rawsign', str(msg)])
  assert res.exit_code == 0
  s = int(safecrypto_test.OneToken(res))
  assert s > 0
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'rsa', 'rawverify', str(msg), str(s)])
  assert res.exit_code == 0 and safecrypto_test.Out(res) == 'RSA signature: OK'
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'rsa', 'rawverify', str(msg + 1), str(s)])
...
def test_elgamal_encrypt_decrypt_and_sign_verify(tmp_path: pathlib.Path) -> None:
  ...
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(base_path), 'elgamal', 'new'])
  ...
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'elgamal', 'rawencrypt', str(msg)])
  assert res.exit_code == 0 and len(safecrypto_test.OneToken(res)) > 0
  ct: str = safecrypto_test.OneToken(res)
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'elgamal', 'rawdecrypt', ct])
  assert res.exit_code == 0 and int(safecrypto_test.OneToken(res)) == msg
  # Sign/verify
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'elgamal', 'rawsign', str(msg)])
  assert res.exit_code == 0 and len(safecrypto_test.OneToken(res)) > 0
  sig: str = safecrypto_test.OneToken(res)
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'elgamal', 'rawverify', str(msg), sig])
  assert res.exit_code == 0 and safecrypto_test.Out(res) == 'El-Gamal signature: OK'
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'elgamal', 'rawverify', str(msg + 1), sig])
  assert res.exit_code == 0 and safecrypto_test.Out(res) == 'El-Gamal signature: INVALID'
...
def test_elgamal_encrypt_decrypt_and_sign_verify_safe(tmp_path: pathlib.Path) -> None:
  ...
  assert res.exit_code == 0 and len(safecrypto_test.OneToken(res)) > 0
  ct_b64: str = safecrypto_test.OneToken(res)
...
def test_dsa_sign_verify(tmp_path: pathlib.Path) -> None:
  ...
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'dsa', 'rawsign', str(msg)])
  assert res.exit_code == 0 and len(safecrypto_test.OneToken(res)) > 0
  sig: str = safecrypto_test.OneToken(res)
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'dsa', 'rawverify', str(msg), sig])
  assert res.exit_code == 0 and safecrypto_test.Out(res) == 'DSA signature: OK'
  tc_logging.ResetConsole()
  app_config.ResetConfig()
  res = safecrypto_test._CallCLI(['-p', str(priv_path), 'dsa', 'rawverify', str(msg + 1), sig])
